...
function AppWithWeb3Storage() {
    ...
    const handleRegisterWithDocuments = async () => {
        try {
            ...
            if (uploadedFiles.deed) {
                documentHashes.deed = await uploadToWeb3Storage(uploadedFiles.deed)
            }
            if (uploadedFiles.survey) {
                documentHashes.survey = await uploadToWeb3Storage(uploadedFiles.survey)
            }
            if (uploadedFiles.photos) {
                documentHashes.photos = await uploadToWeb3Storage(uploadedFiles.photos)
            }
            ...
            const metadataHash = await uploadToWeb3Storage(metadataFile)
    ...
    const registerWithDocumentsTab = (
        <Row gutter={[16, 16]}>
            <Col xs={24}>
                <Card title="Register Land with Documents (Web3.Storage)" bordered>
                    <Form form={documentForm} layout="vertical" disabled={!connected}>
                        ...
                        <Divider>Document Uploads (Web3.Storage)</Divider>
                        ...
                        {uploading && (
                            <div style={{ marginBottom: 16 }}>
                                <Text>Uploading to Web3.Storage...</Text>
                ...
                <Row align="middle" justify="space-between" gutter={[12, 12]}>
                    <Col>
                        <Title level={3} style={{ color: "#fff", margin: 0 }}>
                            Veridia Land Registry + Web3.Storage
                        </Title>
                    </Col>
...
export default AppWithWeb3Storage
